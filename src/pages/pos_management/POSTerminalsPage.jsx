...
export function POSTerminalsPage() {
  ...
  const canManageTerminals = !isCashier;
  ...
  const loadOutlets = async () => {
    try {
      ...
      setOutlets(list);
      if (list.length > 0) {
        const defaultId = user?.outlet_id || list[0].id;
        setSelectedOutletId(defaultId.toString());
      }
    } catch (err) {
      toast.addToast(err.message || "Failed to load outlets", "error");
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingTerminal) {
        await updateTerminalAPI(editingTerminal.id, {
          name: formName.trim(),
          code: formCode.trim(),
        });
        toast.addToast("Terminal updated successfully", "success");
      } else {
        await createTerminalAPI({
                  ...
                  {canManageTerminals && (
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        ...
                        className="flex h-8 w-8 items-center justify-center rounded-xl bg-red-50 text-red-500 hover:bg-red-100 hover:text-red-700 transition"
                        title="Deactivate"
                      >
